...
auto HealthBar(float x, float y, float w, float h, int phealth, bool Outlined) -> VOID
{
	...
	int healthValue = max(0, min(phealth, 100)); // Ограничили значение от 0 до 100

	if (healthValue <= 0) // Если значение здоровья отрицательное или ноль, то ничего не выводим
		return;

	ImColor barColor = ImColor
	(
		min(255 * (100 - healthValue) / 100, 255), min(255 * healthValue / 100, 255), 25, 255
	);
...
auto TabButton(const char* label, int* index, int val, bool sameline) -> VOID
{
	...
	ImVec4 hoverButtonColor = ImColor(128, 128, 128);  // Цвет при наведении мыши
	ImVec4 activeButtonColor = ImColor(128, 128, 128); // Цвет при нажатии

	ImVec4* buttonColor = *index == val ? &activeButtonColor : &defaultButtonColor;
	...
	if (ImGui::Button(label, ImVec2(110, 30)))
		*index = val;
